...
layout:     "post"
tags:       ["project","rt-thread","stm32","运动控制","伺服电机","图像处理","webots","OpenMV"]
---

- [源码仓库](https://github.com/xym-ee/tracking_laser_pointer)
...
来源于 2023 年大学生电子设计竞赛的题目。我的导师正好是电赛的指导老师，我参加过2019年电赛，然后就帮忙一起指导一下。我主要出设计方案，完成控制算法可行性验证，主要是 webots 仿真和实际调试。
...
红色激光笔发射的光斑用来模拟运动目标，光斑落在正前方距离 $ 1 \text{m} $ 处的白色屏幕上，光斑直径 $ \le 1 \text{cm} $。红色光斑位置控制系统控制光斑能在屏幕范围内任意移动。绿色激光笔发射的光斑由绿色光斑位置系统控制，用于自动追踪屏幕上的红色光斑，指示目标的自动追踪效果。$ \le 1 \text{cm} $。
...
首先用一个数学问题的方式来描述，云台和屏幕的几何位置关系如图。
- 相机坐标系 $ \{ C \} $
- 屏幕坐标系 $ \{ T \} $
- 云台坐标系 $ \{ B \} $
